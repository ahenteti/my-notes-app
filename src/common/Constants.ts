import { Color } from './models/Color';

export const HOME_SCREEN_NAME = 'Home';
export const ADD_NOTE_SCREEN_NAME = 'Add Note';
export const CONSULT_NOTE_SCREEN_NAME = 'Consult Note';
export const EDIT_NOTE_SCREEN_NAME = 'Edit Note';

export const HEADER_HEIGHT = 55;
export const HEADER_BACKGROUND_COLOR = new Color('#FFF', '#262A2D');
export const BODY_BACKGROUND_COLOR = new Color('#EEE', '#1C1F23');
export const TEXT_COLOR = new Color('#444', '#EEE');
export const PRIMARY_COLOR = new Color('#3282C9', '#1e96fc');
export const DELETE_BUTTON_BACKGROUND_COLOR = new Color('#e5383b', '#e5383b');
export const DELETE_BUTTON_COLOR = new Color('#EEE', '#EEE');
